...
class ProtoNetAlignQGPASR(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        if self.use_transformer:
            ...
            prototypes2 = torch.chunk(prototypes_all_post, prototypes_all_post.shape[1], dim=1)  # 更新原型
            prototypes3 = torch.chunk(prototypes_all_two, prototypes_all_two.shape[1], dim=1)  # 更新原型
            #prototypes_new = torch.chunk(prototypes_all_two, prototypes_all_two.shape[1], dim=1)
            similarity1 = [self.calculateSimilarity(query_feat, prototype, self.dist_method) for prototype in prototypes]#计算query和原型的相似性
            similarity2 = [self.calculateSimilarity_trans(query_feat, prototype.squeeze(1), self.dist_method) for
                           prototype in prototypes2]
            similarity3 = [self.calculateSimilarity_trans(query_feat, prototype.squeeze(1), self.dist_method) for
                           prototype in prototypes3]
            # 计算每个对应的相似度预测值中的最大值
            #pred = [self.calculatemax(pred1, pred2, pred3) for pred1, pred2, pred3 in
                   # zip(similarity1, similarity2, similarity3)]
            #query_pred=torch.stack(pred, dim=1)
            query_pred1 = F.softmax(torch.stack(similarity1, dim=1), dim=1)
            query_pred2 = F.softmax(torch.stack(similarity2, dim=1), dim=1)
            query_pred3 = F.softmax(torch.stack(similarity3, dim=1), dim=1)
            #print("query_pred1",query_pred1.size())
            all_preds = torch.stack((query_pred1, query_pred2, query_pred3), dim=0)  # Shape: [3, 2, 3, 2048]
            query_pred = torch.mean(all_preds, dim=0)
            # 在第一维上（原型类的维度）计算最大值
            #query_pred, _ = torch.max(all_preds, dim=0)  # max_pred.shape = [2, 3, 2048]
            #all_preds = torch.stack((query_pred1, query_pred2, query_pred3),
                                  #  dim=1)  # Shape: [batch_size, 3, n_prototypes, num_points]

            #print("all_preds", all_preds.size())#all_preds torch.Size([2, 3, 3, 2048])
            #pred = [self.calculatemax(pred1, pred2, pred3) for pred1, pred2, pred3 in
                    #zip(query_pred1, query_pred2, query_pred3)]
            #query_pred = torch.stack(pred, dim=1)
            #print("query_pred", query_pred.size())
            # Get the maximum values along the new dimension (dim=1)
            #query_pred, _ = torch.max(all_preds, dim=1)
            #print("query_pred", query_pred1.size())

            #query_pred = torch.stack(similarity, dim=1)
            #print("query_y", query_y.size())
    ...
    def alignLoss_trans(self, qry_fts, pred, supp_fts, fore_mask, back_mask):
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                loss = loss + F.cross_entropy(supp_pred, supp_label.unsqueeze(0), ignore_index=255) / n_shots / n_ways
        return loss
    def calculatemax(self,pred1, pred2, pred3):
        print("pred1",pred1.size())#pred1 torch.Size([2, 2048])

        pred_all=torch.stack((pred1, pred2, pred3),dim=1)
        print("pred_all", pred_all.size())
        pred,_=torch.max((pred_all),dim=1)
