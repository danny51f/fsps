...
from models.FEM import FEM,FEM_5,FEM_15
...
class ProtoNetAlignQGPASR(nn.Module):
    def __init__(self, args):
        ...
        self.layer55 = nn.Sequential(
            nn.Conv1d(in_channels=320*2, out_channels=320, kernel_size=3, stride=1, padding=2, dilation=2,
                      bias=True),
            nn.BatchNorm1d(320),
            nn.ReLU()
        )
        ...
        self.layer56 = nn.Sequential(
            nn.Conv1d(in_channels=320, out_channels=320, kernel_size=3, stride=1, padding=1, dilation=1,
                      bias=True),
            nn.BatchNorm1d(320),
            nn.ReLU()
        )
        self.residule1 = nn.Sequential(
            nn.Conv1d(321, 320, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv1d(320, 320, kernel_size=3, padding=1)
        )
        ...
        # 假设最终特征转换和分类使用的卷积
        self.layer6 = nn.Conv1d(320, 320, kernel_size=1)  # 特征转换
        self.layer7 = nn.Conv1d(320, 320, kernel_size=1)  # 进一步处理
        self.layer9 = nn.Conv1d(320, self.n_way+1, kernel_size=1)  # 最终分类层
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        query_feat,support_feat=self.enhance(query_feat,support_feat)
        ...
        #print("support_fg_feat",support_fg_feat.size())
        # prototype learning
        ...
        if self.use_transformer:
            ...
            prototypes_all_two = self.transformer(query_feat, support_feat_, prototypes_all_post)
            prototypes_ju=(prototypes_all+prototypes_all_post+prototypes_all_two)/3
            #prototypes_ju = prototypes_all_two
            #print("prototypes_ju", prototypes_ju.size())
            prototypes_new = torch.chunk(prototypes_ju, prototypes_ju.shape[1], dim=1)  # 更新原型
            #print("prototypes_new",prototypes_new.size())
            #prototypesmap=torch.stack(prototypes_ju, dim=0).unsqueeze(0).repeat(query_feat.shape[0], 1, 1)
            ...
            _, selection_map = torch.max(query_pred1, dim=1)  # [2, 2048]
            #print("selection_map",selection_map.size())

            # 步骤 4: 分配最相似原型特征
            allocate_map = []
            for i in range(prototypes_ju.size(0)):  # 遍历批次
                selected_prototypes = prototypes_ju[i, selection_map[i], :]  # 选择最相似的原型 [2048, 320]
                allocate_map.append(selected_prototypes)

            allocate_map = torch.stack(allocate_map, dim=0)  # [2, 2048, 320]
            #print("allocate_map", allocate_map.size())
            allocate_map = allocate_map.transpose(1, 2)  # [2, 320, 2048]

            similarity_map = query_pred1.sum(dim=1).unsqueeze(1)#
            #print("similarity_map", similarity_map.size())#similarity_map torch.Size([2, 1, 2048])
            exit_feat_in = torch.cat([query_feat, allocate_map], dim=1)
            #print("exit_feat_in1", exit_feat_in.size())#exit_feat_in1 torch.Size([2, 640, 2048])
            exit_feat_in = self.layer55(exit_feat_in)
            #print("exit_feat_in2", exit_feat_in.size())#exit_feat_in1 torch.Size([2, 640, 2048])exit_feat_in2 torch.Size([2, 320, 2048])
            exit_feat_in = self.layer56(exit_feat_in)#torch.Size([2, 320, 2048])
            out = torch.cat([exit_feat_in, similarity_map], dim=1)  # [2, 321, 2048]
            #print("out", out.size())
            # 应用残差模块
            out_plus_history = exit_feat_in + self.residule1(out)
            out = out_plus_history + self.residule2(out_plus_history)
            out = out + self.residule3(out)
            #print("out", out.size())out torch.Size([2, 320, 2048])
            similarity2 = [self.calculateSimilarity_trans(out, prototype.squeeze(1), self.dist_method) for
           ...
           out = self.layer6(out)
            out = self.layer7(out)
            out = self.layer9(out)
            #print("out",out.size())#exit_feat_in([2, 3, 2048])
            # 应用 softmax 得到最终的分割结果
            out_softmax = F.softmax(out, dim=1)
            # print("out_softmax", out_softmax.size())  # exit_feat_inout torch.Size([2, 3, 2048])
            out_softmax = torch.argmax(out_softmax, dim=1)
            #print("out_softmax", out_softmax.size())  # exit_feat_inout torch.Size([2, 2048])
            #loss = self.computeCrossEntropyLoss(query_pred, query_y)#proto更新后，计算query和label的损失
            '''
            loss = self.computeCrossEntropyLossxin(query_pred, query_y)
        ...
        return query_pred, loss + align_loss , prototypes_all_post
    ...
    def getPrototype(self, fg_feat, bg_feat):
        ...
        #print("fg_prototypes",fg_prototypes.size())
    ...
    def computeCrossEntropyLossxin(self, query_logits, query_labels, weight_for_background=1.5):
        """ Calculate the CrossEntropy Loss for query set with weight for background class
        """
        # 创建权重向量
        weights = torch.ones(self.n_way+1).to(device=query_logits.device)  # 默认每个类别权重为1
        weights[0] = weight_for_background  # 假设背景类的标签为0
