...
from models.attention import SelfAttention, QGPA,Dualalign
...
class ProtoNetAlignQGPASR(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        if self.use_transformer:
            ...
            query_up = self.Dual(query_feat, support_feat_)
            prototypes_all_post = self.transformer(query_up, support_feat_, prototypes_all)
            #query_up=self.Dual(query_feat,support_feat_)
            ...
            #print("query_feat", query_feat.size())
            # 将 query 映射到相同的维度
            query_up_two = self.Dual(query_up, support_feat_)
            prototypes_all_two = self.transformer(query_up_two, support_feat_, prototypes_all_post)
            #query_up_two = self.Dual(query_up, support_feat_)
            #prototypes_all_three = self.transformer(query_up_two, support_feat_, prototypes_all_two)
            #query_up_three = self.Dual(query_feat, support_feat_)
